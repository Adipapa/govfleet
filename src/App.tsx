...
export function App() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      setDataLoading(true);
      try {
        const [vehicleResponse, telemetryRows, alertResponse] = await Promise.all([
  ...
  const handleAcknowledgeAlert = async (alertId: string) => {
    console.warn('Alert acknowledgement requires the live alert API; no local simulation is performed.', alertId);
  };
  ...
  const handleReplayVehicleTrip = (vehicle: Vehicle) => {
    const existingTrip = trips.find((trip) => trip.vehicleReg === vehicle.regNumber);
    if (existingTrip) { setIsDossierOpen(false); setActivePlaybackTrip(existingTrip); }
  };
  ...
  return (
    <div className="flex flex-row h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden font-sans select-none">
      <Sidebar activeTab={activeTab} setActiveTab={setActiveTab} selectedAgency={effectiveAgency} setSelectedAgency={canSelectAnyAgency ? setSelectedAgency : () => undefined} currentRole={(user.roles[0] || 'fleet_admin') as UserRole} setCurrentRole={() => undefined} activeAlerts={alerts} alertCount={alerts.length} onOpenAlerts={() => setActiveTab('alerts')} onOpenMobileApp={() => setIsMobileModalOpen(true)} onTriggerSOS={() => undefined} onSimulateSOS={() => undefined} onTriggerFuelTheft={() => undefined} onSimulateFuelTheft={() => undefined} simulationSpeed={1} setSimulationSpeed={() => undefined} />
      <main className="flex-1 flex flex-col overflow-hidden relative min-w-0 bg-slate-950">
        ...
        {activeTab === 'fuel' && <FuelMonitoringView vehicles={filteredVehicles} fuelLogs={fuelLogs} onTriggerFuelTheft={() => undefined} onTriggerRefuel={() => undefined} onSelectVehicle={handleSelectVehicle} />}
