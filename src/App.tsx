...
import { ApiUser, getAccessToken, getCurrentUser, getVehicles, getLatestTelemetry, getAlerts, mapVehicle, subscribeToFleetEvents, logout } from './services/api';
...
export function App() {
  ...
  const [fuelLogs] = useState<FuelLogEvent[]>([]);
  const [auditLogs] = useState<AuditLog[]>([]);
  const [geofences] = useState<Geofence[]>([]);
  const [trips] = useState<Trip[]>([]);
  const [maintenance] = useState<MaintenanceItem[]>([]);
  ...
  useEffect(() => {
    ...
    void getCurrentUser().then((currentUser) => setUser(currentUser)).catch(() => setUser(null)).finally(() => setAuthLoading(false));
  }, []);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      setDataLoading(true);
      try {
        const [vehicleResponse, telemetryRows, alertResponse] = await Promise.all([getVehicles({ page: 1, limit: 100 }), getLatestTelemetry(), getAlerts({ page: 1, limit: 100 })]);
        if (cancelled) return;
        const telemetryByVehicle = new Map(telemetryRows.map((row: any) => [row.vehicle_id, row]));
        setVehicles(vehicleResponse.data.map((row: any) => mapVehicle(row, telemetryByVehicle.get(row.id))));
        setAlerts(alertResponse.data as AlertEvent[]);
      } catch (error) {
        if (!cancelled) console.error('Failed to load live fleet data:', error);
      } finally {
        if (!cancelled) setDataLoading(false);
      }
    };
    void refresh();
    const unsubscribe = subscribeToFleetEvents((event: any) => {
      if (event?.type === 'telemetry.updated' || event?.type === 'vehicle.updated' || event?.type === 'alert.created') void refresh();
    });
  ...
  const handleAuthenticated = async () => {
    try { const currentUser = await getCurrentUser(); setUser(currentUser); setActiveTab('overview'); } catch { setUser(null); }
  };
  ...
  const handleAcknowledgeAlert = async (alertId: string) => { console.warn('Alert acknowledgement requires the live alert API; no local simulation is performed.', alertId); };
  const handleDispatchPolice = (alertId: string) => console.warn('Police dispatch requires a live backend workflow.', alertId);
  const handleAddGeofence = (_geo: Geofence) => console.warn('Geofence creation is not connected to the live API yet.');
  const handleDeleteGeofence = (_id: string) => console.warn('Geofence deletion is not connected to the live API yet.');
  const handleAddMaintenance = (_item: MaintenanceItem) => console.warn('Maintenance creation is not connected to the live API yet.');
  const handleReplayVehicleTrip = (vehicle: Vehicle) => { const existingTrip = trips.find((trip) => trip.vehicleReg === vehicle.regNumber); if (existingTrip) { setIsDossierOpen(false); setActivePlaybackTrip(existingTrip); } };
  ...
  return <div className="flex flex-row h-screen w-screen bg-slate-950 text-slate-100 overflow-hidden font-sans select-none">
    ...
    <main className="flex-1 flex flex-col overflow-hidden relative min-w-0 bg-slate-950">
      ...
      {activeTab === 'fuel' && <FuelMonitoringView vehicles={filteredVehicles} fuelLogs={fuelLogs} onSelectVehicle={handleSelectVehicle} />}
    ...
    {isDossierOpen && selectedVehicle && <VehicleDetailModal vehicle={selectedVehicle} onClose={() => setIsDossierOpen(false)} onTriggerSOS={() => undefined} onTriggerFuelTheft={() => undefined} onTriggerRefuel={() => undefined} onPlayTrip={handleReplayVehicleTrip} />}
    ...
    {isMobileModalOpen && <DriverMobileModal vehicle={selectedVehicle || filteredVehicles[0] || vehicles[0]} onClose={() => setIsMobileModalOpen(false)} onTriggerSOS={() => undefined} />}
    {dataLoading && <div className="fixed bottom-4 right-4 z-[9999] rounded-lg border border-slate-700 bg-slate-900/95 px-3 py-2 text-xs text-slate-400 shadow-xl">Loading live fleet data…</div>}
  </div>;
}
