...
export const MapView: React.FC<MapViewProps> = ({
  ...
  const [activeTileProvider, setActiveTileProvider] = useState<TileProvider>('dark');
  ...
  useEffect(() => {
    ...
    const initialTile = TILE_LAYERS.dark;
    ...
    vehicles.forEach((veh) => {
      ...
      const markerHtml = `
        <div class="relative cursor-pointer transition-transform ${isSelected ? 'scale-125 z-50' : 'hover:scale-110'}">
          ...
          <!-- Vehicle Label Tag -->
          <div class="absolute top-8 left-1/2 -translate-x-1/2 whitespace-nowrap px-1.5 py-0.5 rounded bg-slate-950/90 border border-slate-700 text-[10px] text-white font-mono shadow-md font-semibold pointer-events-none">
      ...
      const popupHtml = `
        <div class="p-2.5 text-slate-900 font-sans min-w-[240px]">
          ...
          <div class="grid grid-cols-2 gap-2 text-xs text-slate-700 mb-3">
            <div>
              <span class="text-slate-400 block text-[10px]">CURRENT SPEED</span>
              <span class="font-mono font-bold text-slate-900 text-sm">${veh.speedKmh} km/h</span>
            </div>
            <div>
              <span class="text-slate-400 block text-[10px]">FUEL PROBE (TRIMAGO)</span>
              <span class="font-mono font-bold text-slate-900">${veh.currentFuelPercentage}% (${veh.currentFuelLiters}L)</span>
            </div>
            <div>
              <span class="text-slate-400 block text-[10px]">ASSIGNED DRIVER</span>
              <span class="font-medium text-slate-900">${veh.assignedDriver.name}</span>
            </div>
            <div>
              <span class="text-slate-400 block text-[10px]">SAFETY SCORE</span>
          ...
          <div class="flex space-x-1.5 pt-1">
            <button id="btn-popup-dossier-${veh.id}" class="flex-1 bg-cyan-700 hover:bg-cyan-800 text-white py-1 px-2 rounded text-xs font-semibold text-center transition-colors">
              Full Dossier
            </button>
          </div>
        </div>
  ...
  return (
    <div className="relative w-full h-full bg-slate-950 overflow-hidden select-none">
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col space-y-2">
        ...
        <div className="bg-slate-900/90 backdrop-blur border border-slate-800 rounded-lg p-1.5 shadow-xl text-xs flex items-center space-x-1.5">
          ...
          <select
            ...
            className="bg-slate-800 text-slate-100 rounded px-2 py-1 font-medium focus:outline-none cursor-pointer border border-slate-700"
          >
        ...
        <div className="bg-slate-900/90 backdrop-blur border border-slate-800 rounded-lg p-1 shadow-xl flex flex-col space-y-1">
          <button
            ...
            className="p-2 hover:bg-slate-800 text-slate-300 hover:text-white rounded transition-colors flex items-center space-x-2 text-xs"
          >
          ...
          <button
            ...
            className={`p-2 rounded transition-colors flex items-center space-x-2 text-xs ${
              showGeofences ? 'bg-cyan-950/60 text-cyan-300' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`p-2 rounded transition-colors flex items-center space-x-2 text-xs ${
              measuringMode ? 'bg-amber-950/80 text-amber-300 border border-amber-700' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
            }`}
          >
      ...
      {measuringMode && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 bg-slate-900/95 border border-cyan-700 rounded-lg px-4 py-2 shadow-2xl text-xs flex items-center space-x-3">
          ...
          <div>
            <span className="font-semibold text-white">Geodesic Ruler: </span>
          ...
          <button
            ...
            className="ml-2 text-slate-400 hover:text-white px-2 py-0.5 bg-slate-800 rounded border border-slate-700"
          >
            Reset
          </button>
        </div>
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-slate-950/85 backdrop-blur border border-slate-800 rounded-lg p-2.5 shadow-xl text-xs text-slate-300 max-w-sm hidden md:block">
