...
export const FleetRegistryView: React.FC<FleetRegistryViewProps> = ({
  ...
  const getStatusBadge = (status: VehicleStatus) => {
    switch (status) {
      ...
      case 'parked':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-800 text-slate-300 border border-slate-700">Parked</span>;
      case 'idling':
      ...
      case 'offline':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-900 text-slate-500 border border-slate-800">Offline</span>;
      default:
    ...
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-950 text-slate-100 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-4 gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-xl font-bold tracking-tight text-white">Government Vehicle Fleet Registry</h2>
          </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            onClick={exportCSV}
            className="flex items-center space-x-2 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700 rounded-lg text-xs font-medium transition-colors"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800/80">
          <span className="text-xs text-slate-400 uppercase font-mono">Total Registered</span>
          <p className="text-2xl font-bold text-white font-mono mt-1">{safeVehicles.length}</p>
        </div>
        <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800/80">
          <span className="text-xs text-slate-400 uppercase font-mono">Active In Operation</span>
          <p className="text-2xl font-bold text-emerald-400 font-mono mt-1">
            {safeVehicles.filter((v) => v.status === 'moving' || v.status === 'stopped' || v.status === 'idling').length}
          </p>
        </div>
        <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800/80">
          <span className="text-xs text-slate-400 uppercase font-mono">Emergency / SOS</span>
          <p className="text-2xl font-bold text-red-400 font-mono mt-1">
            {safeVehicles.filter((v) => v.status === 'emergency').length}
          </p>
        </div>
        <div className="p-4 bg-slate-900/60 rounded-xl border border-slate-800/80">
          <span className="text-xs text-slate-400 uppercase font-mono">Avg Safety Rating</span>
                  ...
                  >
                    <td className="p-3">
                      <div className="flex items-center space-x-2.5">
                        ...
                        <div>
                          <p className="font-mono font-bold text-white tracking-wide text-xs group-hover:text-cyan-300">
                            {v.regNumber}
                          </p>
                    ...
                    <td className="p-3">
                      <div className="flex items-center space-x-1.5">
                        ...
                        <span className="text-slate-300 font-medium">{v.department}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-3">
                      <div>
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="font-medium text-slate-200">{v.assignedDriver.name}</span>
                        </div>
                    ...
                    <td className="p-3 text-right">
                      <div className="flex items-center justify-end space-x-1.5" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="p-1.5 bg-slate-800 hover:bg-cyan-950 hover:text-cyan-300 text-slate-300 border border-slate-700 rounded transition-colors"
                        >
