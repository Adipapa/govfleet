...
export const GpsDevicesView: React.FC<GpsDevicesViewProps> = ({ vehicles = [] }) => {
  ...
  /* Live device commands must be backed by a connected device gateway; no simulated acknowledgements. */
  /*
    {
      id: 'log-001',
      timestamp: '11:42:15',
  ...
  */
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50 text-slate-900 space-y-6 max-w-7xl mx-auto w-full">
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="p-3 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-sm font-bold text-slate-900 font-mono mt-1">SinoTrack Global 4G LTE</p>
        ...
        <div className="p-3 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-sm font-bold text-slate-900 font-mono mt-1">12V/24V Cut-Off Relay</p>
        ...
        <div className="p-3 bg-white rounded-xl border border-slate-200">
          ...
          <p className="text-sm font-bold text-slate-900 font-mono mt-1">DIN 2 Physical Duress</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold uppercase font-mono text-slate-500 tracking-wider">
                ...
                <div
                  ...
                  className={`p-3 rounded-lg border cursor-pointer transition-all ${
                    isSelected
                      ...
                      : 'bg-slate-50 border-slate-200/80 hover:bg-slate-100/40'
                  }`}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="font-mono font-bold text-xs text-slate-900">IMEI: {veh.deviceId}</span>
                      ...
                      <p className="text-[11px] text-slate-500 truncate max-w-[200px]">{veh.hardwareModel || 'SinoTrack Telematics'}</p>
                    </div>
                  ...
                  <div className="mt-2 grid grid-cols-3 gap-1 py-1 px-2 bg-white rounded text-[10px] font-mono text-slate-500">
                    <div>4G: <span className="text-slate-900 font-bold">{veh.gsmSignal}%</span></div>
                    <div>Sats: <span className="text-slate-900 font-bold">{veh.satellites}</span></div>
                    <div>VCC: <span className="text-slate-900 font-bold">{veh.batteryVoltage}V</span></div>
                  </div>
                </div>
              );
            })}
        ...
        <div className="lg:col-span-2 space-y-4">
          {selectedVehicle ? (
            <div className="bg-white/80 rounded-xl border border-slate-200 p-5 space-y-5">
              {/* Unit Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Vehicle: <span className="text-slate-900 font-semibold">{selectedVehicle.regNumber}</span> ({selectedVehicle.make} {selectedVehicle.model}) • {selectedVehicle.department}
                  </p>
                </div>

                {/* Sub-tabs */}
                <div className="flex items-center bg-slate-950 border border-slate-200 rounded-lg p-0.5 text-xs font-mono">
                  <button
                    onClick={() => setActiveTab('console')}
                    className={`px-3 py-1 rounded-md transition-colors ${
                      activeTab === 'console' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                    OTA Console
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-md transition-colors ${
                      activeTab === 'wiring' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                    Wiring & Pinout
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-md transition-colors ${
                      activeTab === 'config' ? 'bg-cyan-500 text-slate-950 font-bold' : 'text-slate-500 hover:text-slate-900'
                    }`}
                  >
                    Parameters
                  </button>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-200">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      ...
                      <button
                        ...
                        className="p-2 bg-slate-950 hover:bg-slate-100 border border-slate-200/80 rounded-lg text-left transition-colors text-slate-200 disabled:opacity-50"
                      >
                        <div className="flex items-center space-x-1.5 font-mono text-xs font-bold text-cyan-300">
                          <RefreshCw className="w-3 h-3 text-cyan-400" />
                          <span>CXZT</span>
                        </div>
                        <p className="text-[10px] text-slate-500 mt-0.5">Check GPS, ACC & Battery Status</p>
                      </button>
                      ...
                      >
                        ...
                        <p className="text-[10px] text-slate-500 mt-0.5">Read APN, Server IP & Port</p>
                      </button>
                      ...
                      >
                        ...
                        <p className="text-[10px] text-slate-500 mt-0.5">Set 10s Live Update Interval</p>
                      </button>
                      ...
                      >
                        ...
                        <p className="text-[10px] text-slate-500 mt-0.5">Soft Reboot SinoTrack Modem</p>
                      </button>
                    </div>
                  </div>
                  ...
                  <form onSubmit={handleCustomSubmit} className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-950 border border-slate-200 rounded-lg px-3 py-2 text-xs font-mono text-cyan-300 placeholder:text-slate-600 focus:outline-none focus:border-cyan-500"
                    />
                    <button
                  ...
                  {/* Live Terminal Log */}
                  <div className="p-3 bg-slate-950 rounded-xl border border-slate-200 space-y-2 font-mono text-xs">
                    <div className="flex items-center justify-between border-b border-slate-200/80 pb-2 text-[11px] text-slate-500">
                        ...
                        <div
                          ...
                          className={`p-2 rounded text-[11px] leading-relaxed ${
                            log.direction === 'outbound'
                              ? 'bg-white/90 text-cyan-300 border-l-2 border-cyan-400'
              ...
              {activeTab === 'wiring' && (
                <div className="space-y-4 text-xs">
                  <div className="p-4 bg-slate-950 rounded-xl border border-slate-200 space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        <div className="w-4 h-4 rounded-full bg-red-500 shrink-0 mt-0.5 shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                        <div>
                          <span className="font-bold text-slate-900 block">Red Wire — Power (+) 9V-80V DC</span>
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        <div className="w-4 h-4 rounded-full bg-white border-2 border-slate-500 shrink-0 mt-0.5" />
                        <div>
                          <span className="font-bold text-slate-900 block">Black Wire — Ground (-) Chassis GND</span>
                          <span className="text-slate-500 text-[11px]">
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        ...
                        <div>
                          <span className="font-bold text-slate-900 block">Orange Wire — ACC Key Ignition (DIN 1)</span>
                          <span className="text-slate-500 text-[11px]">
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        ...
                        <div>
                          <span className="font-bold text-slate-900 block">Yellow Wire — Relay Immobilizer (DOUT 1)</span>
                          <span className="text-slate-500 text-[11px]">
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        ...
                        <div>
                          <span className="font-bold text-slate-900 block">Green / White Wire — SOS Panic Wire (DIN 2)</span>
                          <span className="text-slate-500 text-[11px]">
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200 flex items-start space-x-3">
                        ...
                        <div>
                          <span className="font-bold text-slate-900 block">External Audio Port — Surveillance Mic</span>
                          <span className="text-slate-500 text-[11px]">
                  ...
                  <div className="p-4 bg-slate-950 rounded-xl border border-slate-200 space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="p-3 bg-white rounded-lg border border-slate-200">
                        ...
                        <span className="text-slate-900 font-bold">8030000 qts.gov.gm</span>
                        <p className="text-[10px] text-slate-500 mt-1 font-sans">GovNet APN for encrypted private SIMs</p>
                      </div>
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200">
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-sans">SinoTrack TCP Protocol default port 8090</p>
                      </div>
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200">
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-sans">Real-time turn-by-turn waypoint telemetry</p>
                      </div>
                      ...
                      <div className="p-3 bg-white rounded-lg border border-slate-200">
                        ...
                        <p className="text-[10px] text-slate-500 mt-1 font-sans">Dispatches audible in-cab beep and police alert</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="p-12 text-center text-slate-500 bg-white/40 rounded-xl border border-slate-200">
