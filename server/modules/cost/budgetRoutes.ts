...
router.get('/budgets', requirePermission('fleet.read'), async (req,res,next)=>{
  try {
    ...
    `,[typeof req.query.agencyId==='string'?req.query.agencyId:null,typeof req.query.departmentId==='string'?req.query.departmentId:null]);
...
router.post('/budgets', requirePermission('fleet.write'), async(req,res,next)=>{
  try{
    ...
    `,[agencyId,departmentId,name,periodStart,periodEnd,Number(fuelBudget),Number(maintenanceBudget),notes,req.auth!.id]);
...
router.get('/budgets/:budgetId/performance', requirePermission('fleet.read'), async(req,res,next)=>{
  try{
    ...
    const actual=await db.query(`
      SELECT
        COALESCE((SELECT SUM(ft.total_cost) FROM fuel_transactions ft JOIN vehicles v ON v.id=ft.vehicle_id WHERE ${scope.clause.replaceAll('v.','v2.').replace('v2.','v2.')} AND ft.occurred_at >= $1 AND ft.occurred_at < ($2::date + INTERVAL '1 day')),0) fuel_actual,
        COALESCE((SELECT SUM(m.actual_cost) FROM maintenance_records m JOIN vehicles v ON v.id=m.vehicle_id WHERE ${scope.clause} AND m.status='completed' AND m.performed_at >= $1 AND m.performed_at < ($2::date + INTERVAL '1 day')),0) maintenance_actual
