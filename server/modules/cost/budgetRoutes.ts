...
router.get('/budgets', requirePermission('fleet.read'), async (req,res,next)=>{
  try {
    const result=await db.query(`SELECT id,agency_id,department_id,name,period_start,period_end,fuel_budget,maintenance_budget,total_budget,notes FROM financial_budgets WHERE ($1::uuid IS NULL OR agency_id=$1) AND ($2::uuid IS NULL OR department_id=$2) ORDER BY period_start DESC`,[typeof req.query.agencyId==='string'?req.query.agencyId:null,typeof req.query.departmentId==='string'?req.query.departmentId:null]);
...
router.post('/budgets', requirePermission('fleet.write'), async(req,res,next)=>{
  try{
    ...
    const result=await db.query(`INSERT INTO financial_budgets(agency_id,department_id,name,period_start,period_end,fuel_budget,maintenance_budget,notes,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,[agencyId,departmentId,name,periodStart,periodEnd,Number(fuelBudget),Number(maintenanceBudget),notes,req.auth!.id]);
...
router.get('/budgets/:budgetId/performance', requirePermission('fleet.read'), async(req,res,next)=>{
  try{
    const budget=await db.query('SELECT * FROM financial_budgets WHERE id=$1',[req.params.budgetId]);
    if(!budget.rows[0])return res.status(404).json({error:'Budget not found'});
    const b=budget.rows[0];
    const scope=vehicleScope(req,'v');
    const shiftScope = scope.clause.replace(/\$(\d+)/g, (_m, n) => `$${Number(n)+1}`);
    const actual=await db.query(`SELECT COALESCE((SELECT SUM(ft.total_cost) FROM fuel_transactions ft JOIN vehicles v ON v.id=ft.vehicle_id WHERE ${shiftScope} AND ft.occurred_at >= $1 AND ft.occurred_at < ($2::date + INTERVAL '1 day')),0) fuel_actual, COALESCE((SELECT SUM(m.actual_cost) FROM maintenance_records m JOIN vehicles v ON v.id=m.vehicle_id WHERE ${shiftScope} AND m.status='completed' AND m.performed_at >= $1 AND m.performed_at < ($2::date + INTERVAL '1 day')),0) maintenance_actual`,[b.period_start,b.period_end,...scope.params]);
